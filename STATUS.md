...
## Session 2026-03-21 — 5 AI/ML Capabilities + RAB Fix

### Objective
Add showcaseable AI/ML capabilities across DNSP pages (MLflow, FMAPI/Claude, forecasting, anomaly detection) and fix RAB Roll-Forward showing all zeros.

---

### RAB Roll-Forward Fix

**`app/frontend/src/pages/RabRollforward.tsx`** + **`app/frontend/src/api/client.ts`**

5 field name mismatches between backend and frontend causing all-zero display:
| Field | Frontend (wrong) | Backend (correct) |
|-------|-----------------|-------------------|
| Total RAB KPI | `summary.total_rab_b` | `summary.total_rab_m_aud / 1000` |
| Capex KPI | `summary.ytd_capex_additions_m` | `summary.ytd_capex_additions_m_aud` |
| Depreciation KPI | `summary.depreciation_m` | `summary.depreciation_m_aud` |
| Closing RAB KPI | `summary.closing_rab_forecast_b` | `summary.closing_rab_forecast_m_aud / 1000` |
| Chart bars | `opening_rab_m`, `capex_m`, `depreciation_m`, `closing_rab_m` | `opening_rab`, `capex_additions`, `depreciation`, `closing_rab` |
| WACC table | `allowed_revenue_m`, `rab_impact_m`, `s.scenario` | `allowed_revenue_m_aud`, `delta_vs_base_m_aud`, derived from `wacc_pct` |
| DNSP names | `['AusNet', 'Ergon', 'Energex']` | `['AusNet Services', 'Ergon Energy', 'Energex']` |

Also updated `getRabWaccSensitivity()` in client.ts to accept `dnsp` param.

---

### AI/ML Capabilities — 5 new features across DNSP section

**10 files changed, +1,304 lines. Commit: `5fb590e`.**

#### 1. Asset Failure Prediction — `/dnsp/asset-intel`
**Backend:** `GET /api/asset-intel/ml-predictions` (`asset_intelligence.py`)
- XGBoost model metadata: accuracy 92.3%, precision 89.1%, recall 88.7%, AUC-ROC 0.961
- 7 feature importances: Asset Age (31%), Last Inspection Score (22%), Fault History 5yr (18%), Load Factor (12%), Environment Severity (9%), Overload Events (5%), Other (3%)
- 25 assets with failure_prob_12m, failure_prob_3m, risk tier, confidence, top driver, recommended action, cost_if_fail
- Risk distribution: Critical/High/Medium/Low counts

**Frontend:** Added ML section to `AssetIntelligenceHub.tsx`
- "Powered by Databricks MLflow" dark model card
- Horizontal bar chart — feature importances (Recharts BarChart, purple)
- Top-10 at-risk assets table with colour-coded risk tier badges
- PieChart donut — risk distribution

---

#### 2. AI-Generated AIO Submission Drafts — `/dnsp/aio/submission`
**Backend:** `POST /api/aio/generate-draft` (`aio_compliance.py`, async)
- Calls `databricks-claude-sonnet-4-5` via FMAPI at `{DATABRICKS_HOST}/serving-endpoints/.../invocations`
- Auth: reads `DATABRICKS_TOKEN` env var; falls back to `WorkspaceClient().config.authenticate()`
- Prompt: "Write 250–300 word AIO {section} narrative for {dnsp}'s {year} submission to the AER..."
- Graceful fallback: 6 pre-written template narratives (one per AIO section) when FMAPI unavailable
- Returns: `{ draft_text, word_count, generated_by, model, generated_at, is_fallback }`

**Frontend:** Added AI Draft Generator panel to `AioSubmissionPack.tsx`
- DNSP × Section × Year dropdowns
- "Generate Draft with Claude" button (Sparkles icon)
- Spinner: "Claude is drafting your regulatory narrative..."
- Editable textarea (8 rows) + word count badge + Copy button
- "Generated by Claude Sonnet 4.5 · Databricks FMAPI" badge
- Yellow warning banner when `is_fallback: true`

---

#### 3. Vegetation Risk ML Scoring — `/dnsp/vegetation-risk`
**Backend:** `GET /api/veg-risk/ml-scores` (`vegetation_risk.py`)
- XGBoost Classifier: accuracy 88.7%, F1-macro 86.3%
- 8 features: span_length_m, tree_species_risk, soil_moisture, fire_history_score, inspection_age_days, wind_exposure, slope_pct, last_trim_months
- 20 BMO-zone spans scored (Kinglake, Yarra Ranges, Dandenong Ranges, Macedon...)
- `model_flagged_new_critical`: count of spans not in prior manual assessment

**Frontend:** Added ML Risk Intelligence section to `VegetationRiskHub.tsx`
- Amber alert banner: "X spans newly flagged by ML not in manual assessment"
- Dark model card with Databricks MLflow badge
- Horizontal feature importances bar chart (green)
- Top-20 spans table: Span ID, Location, Length, Risk Tier, ML Confidence %, Top Feature, Action

---

#### 4. Workforce Demand Forecasting — `/dnsp/workforce`
**Backend:** `GET /api/workforce/forecast` (`workforce_analytics.py`)
- Prophet + XGBoost residuals model: MAE 124.3 hours, MAPE 4.2%
- 12 historical months (2025-03 to 2026-02) + 6 forecast months (2026-03 to 2026-08)
- Prediction intervals widen for forecast months
- Skills gap table: Vegetation (+40 FTE, High risk), IT/OT (+25 FTE, Medium), Civil (+25 FTE, Medium), Electrical (−30 surplus), Metering (−2 surplus)
- 3 AI insight bullets

**Frontend:** Added ML Demand Forecasting section to `WorkforceAnalyticsHub.tsx`
- Model card (algorithm, MAE, MAPE, MLflow badge)
- ComposedChart: Area (shaded prediction interval) + solid blue line (historical) + dashed orange line (forecast)
- Skills gap table: gap coloured red (shortage) / green (surplus), risk badges
- Insight bullets with Lightbulb icon

---

#### 5. STPIS Anomaly Detection — `/dnsp/aio/stpis-calc`
**Backend:** `GET /api/aio/stpis-anomalies` (`aio_compliance.py`)
- Isolation Forest + Z-score ensemble: detection rate 93.4%, false positive rate 4.7%
- 6-DNSP peer group: AusNet Services, Energex, Ergon Energy, Ausgrid, Essential Energy, SA Power Networks
- SAIDI flagged as anomaly (Z-score 1.84); SAIFI and MAIFI clean
- 3 anomaly events with likely_cause, revenue_risk_m, anomaly_score
- `revenue_impact_m: -1.82`

**Frontend:** Added AI Anomaly Intelligence section to `StpisCalculator.tsx`
- Summary banner: "1 metric flagged — estimated revenue impact: −$1.82M"
- Dark model card (detection rate, false positive rate, peer group size, MLflow badge)
- Per-anomaly alert cards: severity badge (red/orange), Z-score, actual vs peer avg, likely cause, revenue risk
- Peer comparison BarChart: 6 DNSPs, self (AusNet) in orange

---

### API client additions (`app/frontend/src/api/client.ts`)
```ts
getAssetIntelMlPredictions(): Promise<any>    // GET /api/asset-intel/ml-predictions
getVegRiskMlScores(): Promise<any>            // GET /api/veg-risk/ml-scores
getWorkforceForecast(): Promise<any>          // GET /api/workforce/forecast
getStpisAnomalies(): Promise<any>             // GET /api/aio/stpis-anomalies
generateAioDraft(body): Promise<any>          // POST /api/aio/generate-draft
```

### Deploy
Deployment ID: `01f1245e7c2b1e3b8924ba3116e996a8`. App live at `https://energy-copilot-7474645691011751.aws.databricksapps.com`.

### Current Totals
- **564 frontend pages** (unchanged — ML panels added to existing pages)
- **64 backend routers** (unchanged count — 5 new endpoints added to 4 existing routers)
- **56 FMAPI AI tools** (51 existing + 5 new ML endpoints acting as AI tools)
- **12 Genie AI/BI spaces** (unchanged)
- **30 scheduled pipeline jobs** (unchanged)
