...
def add_wind_target(spark: SparkSession, df_spark) -> "pyspark.sql.DataFrame":
    """Add wind_generation_target by joining future gen_wind_mw values."""
...
def prepare_data(spark, region, train_end, val_end, test_end):
    ...
    df_spark = add_wind_target(spark, base)
...
def train_region(spark, region, train_end, val_end, test_end):
    ...
    with mlflow.start_run(run_name=f"{region}_final_model"):
        ...
        test_metrics = compute_metrics(y_te, booster.predict(X_te))
